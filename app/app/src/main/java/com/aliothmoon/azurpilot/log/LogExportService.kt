...
enum class LogExportKind(val filePrefix: String) {
    /** AzurPilot 日志（官方 issue 格式：zip 条目镜像 `log/xxx` 相对路径，只收近 7 天） */
...
class LogExportService(
    ...
    /** AzurPilot 日志目录（`rootfs/opt/run/log`）；zip 条目镜像成官方 issue 的 `log/xxx` */
    ...
    private fun createShareIntent(zip: File): Intent {
        ...
        return Intent(Intent.ACTION_SEND).apply {
            ...
            putExtra(Intent.EXTRA_SUBJECT, context.getString(com.aliothmoon.azurpilot.R.string.log_export_subject))
