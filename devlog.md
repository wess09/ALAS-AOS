...
- **桌面 ALAS 未被我改动（证据链）**：本会话完整命令记录对 `C:\other\AzurLaneAutoScript` 零引用；其 git 已跟踪文件零修改（HEAD 92c07aa 是其自带热更新所升）；toolkit imageio=2.27.0（上游钉版，site-packages 最新文件 2023-06）；`config/alas.json` mtime 9-12。PC 对照实验全部在项目内 `.tmp/venv-gif*`（系统 Python 3.14 venv）跑，素材取项目内 `.tmp/alas` 副本。用户 20:17 看到的"电脑上操作 ALAS"=我 `start http://127.0.0.1:22267` 用浏览器遥控**手机** WebUI。
...
- **用户实测确认「功能完好，可以照常运行」**——工具栏独立开启全案最后两项遗留验收全部通过：
  - **D10 半自动点击手动终验**：daemon 预启动链（runner 先 `alas.run('start')` 拉游戏过登录再进盯屏）实战有效，手动配合半自动点击正常。
...
- **共享面板**：`AlasControlPanel` 加 `showTools` 开关（默认 true）——悬浮窗保留原工具区（含标签），挂机页传 false 用新行。
...
- **验证 1**：CDP（`webview_devtools_remote_<pid>` + Runtime.evaluate）计算样式实证——spinner `animName:none`、`borderRight==borderLeft`、wrapper `borderTopStyle:none`；两帧截图像素级零差异；目视「Alas ◯ 闲置」完整静态灰圆、方框消失。**用户目视终验通过**。
...
- **`app/README.md`**（上游 MaaFwApp README）头部标注「上游存档，不描述本仓库产品」——它还写着「分辨率可选 720P/1080P」这类 fork 已删功能，防误导。
